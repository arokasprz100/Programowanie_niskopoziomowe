...
Należy napisać 3 krótkie funkcje tak, by osiągnąć 
...
void breakMemory() {
	VALGRIND_PRINTF_BACKTRACE("Here we add some errors");
	int x;
	if (x) { x = 1; }
}
...
void printNumberOfErrors() {
	...
	if (numberOfErrors) {
		std::cout << "We have detected " << numberOfErrors << " memory errors." << std::endl;
	}
	else {
